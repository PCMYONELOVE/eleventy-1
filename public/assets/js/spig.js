...
$(document).on('copy',function(){
    showMessage('Что вы скопировали? Перепечатано, чтобы не забывать добавлять источник.',8000);
});
...
jQuery(document).ready(function($){
    ...
    if(window.location.href == SiteIndexUrl){
        ...
        if(now>23||now<=5){
            text = 'Ты сова? Я все еще не спим так поздно, приходите завтра.';
        }else if(now>5&&now<=7){
            text = 'Доброе утро! День уже утром, хороший день вот-вот начнется.';
